...
如果你还记得上一篇的结尾，执行 `dskcode chat` 只会看到一行干巴巴的 `dskcode chat — 待实现（第07章）`。这显然不行——一个标榜"AI 编程助手"的 CLI 工具，启动后连个像样的界面都没有，谁会用？

这一篇的目标很简单：

- `dskcode --help` 输出带颜色，至少别那么丑
- `dskcode chat` 启动后有一个可交互的终端界面
- 用户能打字、能看到消息、能敲 `/help`
...
2. 关键信息着色：`dskcode` 青色、`<command>` 绿色、注释灰色
...
注意 `render()` 返回的 `waitUntilExit` 是一个函数，调用后返回 Promise。我把它包装成属性，这样调用方可以用 `app.waitUntilExit` 等待渲染结束。这在 `dskcode chat` 的 action 中会用到。

### DskcodeSplash：会变色的 ASCII Logo

这个组件只做一件事：显示 DSKCODE 的 ASCII logo，然后每隔 500ms 让颜色在下标组里循环：
...
在 `index.tsx` 中把 `dskcode chat` 的 action 从 `console.log` 改为渲染 ChatSession：
...
1. **`async function` 而非箭头函数**——因为要访问 `this` 拿到 `dskcodeCtx`。箭头函数没有自己的 `this`，不能用。
...
`dskcode chat` 启动后，你会看到：

1. 一个带边框和彩虹颜色跑马的 DSKCODE ASCII logo
...
- 组件可复用，Spinner 和 StatusMessage 后续在 `dskcode run` 和 `dskcode setup` 中也能用
- 状态管理干净，随时加新功能（比如流式输出 AI 回复）代码结构不会乱
- 社区生态成熟，有 100+ 第三方 Ink 组件

如果只是简单的日志输出，`console.log` + chalk 足够了。但 `dskcode chat` 是一个交互式会话，需要跑马灯、输入框、消息列表，Ink 是合理的选择。
...
下一章我们来实现配置系统——TOML 解析、多层级合并、环境变量覆盖，让 dskcode 真正可用。
